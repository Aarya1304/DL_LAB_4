...
def train_one_trial(cell_class, trial_id, save_root, units=128, num_layers=1,
    ...
    # random seed for reproducibility, just to be safe
    tf.random.set_seed(seed)
    np.random.seed(seed)

    # load dataset, let's do it
    ds_train, ds_val, (seq_len, input_dim) = prepare_notmnist_dataset(
        batch_size=batch_size, seed=seed+trial_id
    )

    # build model, easy peasy
    model = build_rnn_model(cell_class, units=units, num_layers=num_layers,
                            sequence_length=seq_len, input_dim=input_dim,
                            num_classes=10, dropout=dropout)
    ...
    # -------------------------
    # Training in do-while style
    # -------------------------
    done_training = False
    while not done_training:  # pseudo do-while
        hist = model.fit(ds_train, validation_data=ds_val, epochs=epochs)
        done_training = True  # finished one training loop

    # save model somewhere, yep
...
# ----------------------------------
def run_experiments(cell_name='GRU', units=128, num_layers=1, trials=3,
                    epochs=20, batch_size=128, lr=0.001, dropout=0.0):

    # switch-case like style
    cell_map = {'GRU': GRUCell, 'MGU': MGUCell}

    def switch_cell(name):
        switch_dict = {
            'GRU': lambda: GRUCell,
            'MGU': lambda: MGUCell
        }
        return switch_dict.get(name, lambda: (_ for _ in ()).throw(ValueError(f"Unknown cell type: {name}. Use 'GRU' or 'MGU'")))()

    cell_class = switch_cell(cell_name)
    save_root = 'models'

    val_accuracies = []

    # do-while style loop for trials
    t = 0
    while t < trials:
